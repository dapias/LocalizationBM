...
###With this we can obtain Tikhonov and Biroli elements
function resolvent_diagonal(lambda::Float64, c::Int64, T::Float64, Np::Int64, ensemble::Int64,epsilon2::Float64, poparray; epsilon = 1e-300)
    ##For this function a population array is passed that is assumed is already equilibrated
    #poparray = init_population(Np)
    #population_update!(lambda, c, T, Np, nsteps, epsilon2, poparray); 
    
    beta = 1.0/T
    ##init arrays
    res = zeros(Complex{Float64}, ensemble);
    #ipr = zeros(ensemble)
    ######
    dist_energy = Exponential()
    random_elements = rand(1:Np,c)
    ...
    sum_term = sum(im*fsym.*zetas_sample./(im*fsym .+ zetas_sample))
    zeta_c =   im*(lambda-epsilon2*im)/(exp(-beta*e1)/c)  + sum_term
    #ipr[1] = (real(1/(zeta_c*(exp(-beta*e1)/c))))^2
    res[1] = 1/(zeta_c*(exp(-beta*e1)/c))
    ##update the population after one measurement
    population_update!(lambda,c,T,Np, 1, epsilon, poparray)
    ##########################
    for j in 2:ensemble
        ...
        sum_term = sum(im*fsym.*zetas_sample./(im*fsym .+ zetas_sample))
        zeta_c =   im*(lambda-epsilon2*im)/(exp(-beta*e1)/c)  + sum_term
        #ipr[j] = (real(1/(zeta_c*(exp(-beta*e1)/c))))^2
        res[j] = 1/(zeta_c*(exp(-beta*e1)/c))
        ##update the population with new values
        population_update!(lambda,c,T,Np, 1, epsilon, poparray)
        ##########################
    end
